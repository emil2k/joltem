...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        """Create fingerprints from each key"""
        for auth in orm.Authentication.objects.all():
            try:
                key = Key.fromString(auth.key)
                if not key.sshType() == 'ssh-rsa':
                    raise BadKeyError("No a rsa key.")
                elif not key.isPublic():
                    raise BadKeyError("Not a public key.")
            except BadKeyError:
                auth.delete()  # delete, invalid keys
            else:
                auth.blob = key.toString('OPENSSH')
