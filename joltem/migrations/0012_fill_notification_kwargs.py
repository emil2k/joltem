...
class Migration(DataMigration):
    ...
    depends_on = (
        ('project', '0012_founder'),
    )

    def forwards(self, orm):
        "Write your forwards methods here."
        for notify in orm.Notification.objects.all():
            ntype = notify.notifying_type
            ntype = models.get_model(ntype.app_label, ntype.model)
            try:
                notifying = ntype.objects.get(pk=notify.notifying_id)
                kwargs = notify.kwargs or {}
                kwargs.update(notifying.get_notification_kwargs(notify))
                notify.kwargs = kwargs
                notify.save()
