...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        """ Delete unnecessary notifications of added comments.

        All comments may have been deleted.

        :param orm:

        """
        for n in orm.Notification.objects.filter(
                type=settings.NOTIFICATION_TYPES.comment_added):
            if not orm.Comment.objects.filter(
                    Q(commentable_type_id=n.notifying_type_id)
